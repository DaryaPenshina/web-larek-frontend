...
`basket: string[]` список объектов продуктов, добавленных в корзину.
...
Поля
`card` карточка товара для предпросмотра

Методы:
`set_card`: Устанавливает текстовое содержимое элемента _text.
...
Класс Order управляет формой оформления заказа. 
